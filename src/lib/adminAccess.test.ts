...
describe('adminAccess — phân quyền truy cập admin panel theo role', () => {
  ...
  describe('visibleTabs', () => {
    ...
    it('staff thấy hồ sơ cá nhân + CRM khách hàng + phiên chat + customer workspace', () => {
      expect(visibleTabs('staff')).toEqual(['agent-profiles', 'leads', 'chat-sessions', 'users']);
    });
    ...
    it('STAFF_TABS khớp tập tab staff', () => {
      expect(STAFF_TABS).toEqual(['agent-profiles', 'leads', 'chat-sessions', 'users']);
    });
  });
