...
export function validateCoordinatePair(latitudeInput: unknown, longitudeInput: unknown): CoordinateValidation {
  ...
  if (latitudeEmpty || longitudeEmpty) return { valid: false, message: 'Vui lòng nhập đủ cả vĩ độ và kinh độ, hoặc xóa cả hai ô.' };

  const latitude = parseCoordinate(latitudeInput);
  const longitude = parseCoordinate(longitudeInput);
  if (latitude === null || longitude === null) return { valid: false, message: 'Tọa độ phải là số hợp lệ.' };
  if (latitude < -90 || latitude > 90) return { valid: false, message: 'Vĩ độ phải nằm trong khoảng -90 đến 90.' };
  if (longitude < -180 || longitude > 180) return { valid: false, message: 'Kinh độ phải nằm trong khoảng -180 đến 180.' };
