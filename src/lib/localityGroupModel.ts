...
function groupIdentity(property: Property, areaId?: string): { key: string; level: LocalityGroupLevel; label: string } {
...
export function buildLocalityGroups(properties: Property[], areaId?: string): LocalityGroup[] {
  const groups = new Map<string, LocalityGroup & { latitudes: number[]; longitudes: number[] }>();
  for (const property of properties) {
    const identity = groupIdentity(property, areaId);
