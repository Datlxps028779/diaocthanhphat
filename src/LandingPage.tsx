...
export function LandingPage({ onNavigate, user, onShowAuth }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--cnv-surface-soft)]">
      ...
      <section className="relative z-10 min-h-[720px] overflow-hidden bg-slate-950 pt-[132px] md:pt-[184px] lg:min-h-[760px]">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-slate-950/35 via-slate-950/35 to-slate-950/80" />
          <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-slate-950 to-transparent" />
        </div>

        <div className="relative z-10 mx-auto flex min-h-[588px] w-full max-w-7xl flex-col items-center justify-center px-4 pb-14 text-center md:min-h-[576px] lg:pb-18">
          <div className="inline-flex items-center gap-2 border border-white/25 bg-white/10 px-3 py-1.5 text-xs font-bold text-white shadow-sm backdrop-blur-md">
            <MapPin className="h-3.5 w-3.5 text-amber-300" />{sec('hero')('hero_label', 'Tập trung khu vực Bình Dương')}
          </div>
          <h1 className="mt-6 max-w-4xl font-display text-4xl font-extrabold leading-[1.08] tracking-tight text-white md:text-6xl lg:text-7xl">
            {sec('hero')('title', 'Tìm kiếm bất động sản tại Bình Dương')}
          </h1>
          <p className="mt-5 max-w-2xl text-sm leading-6 text-white/85 md:text-lg md:leading-8">
            {sec('hero')('subtitle', 'Khám phá tài sản theo vị trí, pháp lý và ngân sách phù hợp với nhu cầu thực.')}
          </p>

          <div className="mt-9 w-full max-w-5xl border border-white/35 bg-white/80 p-4 text-left shadow-[0_30px_100px_rgba(15,23,42,0.28)] backdrop-blur-xl md:p-6">
            <div className="mb-4 flex gap-1 border-b border-white/50 pb-4">
              {LISTING_TYPE_TABS.map(tab => (
                <button
                  key={tab.key}
                  onClick={() => { setActiveTab(tab.key); setSearchPriceIdx(0); }}
                  className={`relative flex-1 px-5 py-3 text-sm font-extrabold transition-all md:flex-none md:min-w-48 ${activeTab === tab.key ? 'bg-red-600 text-white shadow-lg shadow-red-900/15' : 'bg-white/70 text-slate-700 hover:bg-white hover:text-red-700'}`}
                >
            ...
            <div className="grid gap-2 lg:grid-cols-[1.35fr_1fr_1fr_1fr_auto]">
              <div className="relative lg:col-span-2">
                ...
                <input
                  ...
                  className="h-14 w-full border border-white bg-white/95 pl-12 pr-4 text-base font-medium text-slate-800 outline-none shadow-sm transition focus:border-red-400 focus:ring-2 focus:ring-red-600/20"
                />
              </div>
              <select
                value={searchAreaId}
                onChange={e => { setSearchAreaId(e.target.value); setSearchDistrict(''); setSearchWard(''); }}
                className="h-14 border border-white bg-white/95 px-4 text-sm font-semibold text-slate-700 outline-none shadow-sm focus:border-red-400 focus:ring-2 focus:ring-red-600/20"
              >
              ...
              <button
                ...
                className="flex h-14 items-center justify-center gap-2 bg-red-600 px-8 text-base font-extrabold text-white shadow-lg shadow-red-900/15 transition-colors hover:bg-red-700"
              >
            ...
            {(searchDistricts.length > 0 || searchWards.length > 0) && (
              <div className="mt-2 grid gap-2 md:grid-cols-2">
                {searchDistricts.length > 0 && (
                  <select
                    value={searchDistrict}
                    onChange={e => { setSearchDistrict(e.target.value); setSearchWard(''); }}
                    className="h-12 border border-white bg-white/90 px-4 text-sm font-semibold text-slate-700 outline-none shadow-sm focus:border-red-400 focus:ring-2 focus:ring-red-600/20"
                  >
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-white md:text-slate-100">
              <span className="mr-1 text-xs font-semibold text-white/80 md:text-slate-600">Tìm nhanh:</span>
              {(activeTab === 'cho_thue' ? PRICE_RANGES_RENT : PRICE_RANGES_SALE).slice(1, 9).map((r, i) => (
                <button key={r.label} onClick={() => setSearchPriceIdx(i + 1)}
                  className={`rounded-full border px-3 py-1.5 text-xs font-bold transition-colors ${searchPriceIdx === i + 1 ? 'border-red-600 bg-red-600 text-white' : 'border-white/70 bg-white/30 text-white hover:bg-white/45 md:border-white md:bg-white/70 md:text-slate-700 md:hover:bg-white'}`}>
                  {r.label}
                </button>
              ))}
              {areas.slice(0, 2).map(a => (
                <button key={a.id} onClick={() => goListings({ listingType: 'mua_ban', areaId: a.id })}
                  className="rounded-full border border-red-100 bg-white/80 px-3 py-1.5 text-xs font-bold text-red-700 transition-colors hover:bg-red-50">
                  {a.name}
                </button>
              ))}
            </div>
          ...
          <div className="mt-8 flex items-center gap-3 text-xs font-bold uppercase tracking-wide text-white/75">
