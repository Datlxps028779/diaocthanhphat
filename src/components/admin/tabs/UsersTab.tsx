...
function CustomerDetailView({ customerId, staff, canManageAssignments, onBack, onChanged }: {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-5">
        <h3 className="font-bold text-gray-900 mb-3">Lead từ tin đăng ({customer.listingLeads.length})</h3>
        <p className="text-xs text-gray-500 mb-3">Lead được tính theo property thuộc các tin của customer; nhân viên active của customer cùng phụ trách phạm vi này.</p>
        <div className="space-y-2 max-h-80 overflow-auto">
          {customer.listingLeads.map(lead => (
            <div key={lead.id} className="border border-gray-100 rounded-lg px-3 py-2 text-sm">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="font-medium">{lead.full_name} · {lead.phone}</div>
                <span className="text-xs text-gray-500">{lead.status}</span>
              </div>
              <div className="text-xs text-gray-400 mt-1">{lead.property_title} · {lead.source || 'Không rõ nguồn'} · {new Date(lead.created_at).toLocaleString('vi-VN')}</div>
              {lead.message && <div className="text-xs text-gray-600 mt-1">{lead.message}</div>}
              {lead.note && <div className="text-xs text-gray-500 mt-1">Note: {lead.note}</div>}
              {lead.follow_up_at && <div className="text-xs text-amber-700 mt-1">Hẹn gọi: {new Date(lead.follow_up_at).toLocaleString('vi-VN')}</div>}
            </div>
          ))}
          {customer.listingLeads.length === 0 && <p className="text-sm text-gray-400">Chưa có lead phát sinh từ tin đăng.</p>}
        </div>
      </div>
