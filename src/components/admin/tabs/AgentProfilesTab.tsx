...
type WorkspaceTab = 'overview' | 'public' | 'activity' | 'history';
...
export function AgentProfilesTab({ role }: Props) {
  ...
  const [selected, setSelected] = useState<AgentProfileDirectoryRow | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      setRows(next);
      setSelected(current => current ? next.find(row => row.profile_id === current.profile_id) ?? null : null);
    } catch (e) {
  ...
  if (selected) {
    return <AgentProfileWorkspace profile={selected} role={role} onBack={() => setSelected(null)} onChanged={load} />;
  }
...
function AgentProfileWorkspace({ profile, role, onBack, onChanged }: { profile: AgentProfileDirectoryRow; role: 'user' | 'staff' | 'admin'; onBack: () => void; onChanged: () => Promise<void> }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-gray-200 mb-5">{([['overview', 'Tổng quan'], ['public', 'Thông tin công khai'], ['activity', 'Hoạt động'], ['history', 'Lịch sử']] as Array<[WorkspaceTab, string]>).map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`px-3 py-2.5 text-sm font-semibold border-b-2 ${tab === key ? 'border-red-600 text-red-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>{label}</button>)}</div>
...
function HistoryView({ audit, loading }: { audit: AgentProfileAuditEvent[]; loading: boolean }) { return <div className="bg-white rounded-2xl border border-gray-100 p-5"><h3 className="font-bold text-gray-900 flex items-center gap-2 mb-4"><History className="w-4 h-4 text-red-500" /> Lịch sử thay đổi</h3>{loading ? <p className="text-sm text-gray-400">Đang tải lịch sử...</p> : audit.length === 0 ? <p className="text-sm text-gray-400">Chưa có lịch sử.</p> : <div className="space-y-3">{audit.map(event => <div key={event.id} className="border-l-2 border-red-100 pl-3"><p className="text-sm font-semibold text-gray-800">{AUDIT_LABEL[event.action]}</p><p className="text-xs text-gray-400">{new Date(event.created_at).toLocaleString('vi-VN')} · {event.actor_id ?? 'system'}</p></div>)}</div>}</div>; }
