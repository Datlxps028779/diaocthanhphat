...
function PublicEditor({ profile, isAdmin, onSaved }: { profile: AgentProfileDirectoryRow; isAdmin: boolean; onSaved: () => Promise<void> }) {
  ...
  const [error, setError] = useState('');
  const slugHasChanged = form.slug.trim() !== profile.slug;
  const slugIsLocked = profile.slug_change_count >= 1;
  useEffect(() => {
    setForm({ slug: profile.slug, display_name: profile.display_name, bio: profile.bio ?? '', avatar_url: profile.avatar_url ?? '', public_phone: profile.public_phone ?? '', public_zalo: profile.public_zalo ?? '', status: profile.status });
  ...
  const save = async () => {
    ...
    try {
      if (slugHasChanged && !confirmSlugChange) throw new Error('Hãy xác nhận cảnh báo trước khi đổi slug hồ sơ.');
      await updateAgentProfile(profile.profile_id, { ...form, status: isAdmin ? form.status : undefined }, slugHasChanged && confirmSlugChange);
      await onSaved();
      setMessage('Đã lưu hồ sơ.');
    } catch (e) { setError(e instanceof Error ? e.message : 'Không lưu được hồ sơ.'); } finally { setBusy(false); }
  };
  return <div className="bg-white rounded-2xl border border-gray-100 p-5 max-w-3xl"><div className="flex items-center gap-2 mb-4"><FileText className="w-4 h-4 text-red-500" /><h3 className="font-bold text-gray-900">Thông tin công khai</h3></div>{error && <p className="text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2 mb-3">{error}</p>}{message && <p className="text-sm text-emerald-700 bg-emerald-50 rounded-lg px-3 py-2 mb-3">{message}</p>}<div className="grid md:grid-cols-2 gap-3"><Field label="Tên hiển thị" value={form.display_name} onChange={value => set('display_name', value)} /><Field label="Slug" value={form.slug} disabled={slugIsLocked} onChange={value => { set('slug', value); setConfirmSlugChange(false); }} />{slugIsLocked ? <p className="md:col-span-2 -mt-2 text-xs text-gray-500">Slug đã được đổi một lần và hiện đã khóa vĩnh viễn.</p> : slugHasChanged && <label className="md:col-span-2 -mt-2 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-800"><input type="checkbox" checked={confirmSlugChange} onChange={e => setConfirmSlugChange(e.target.checked)} className="mt-0.5" /><span>Tôi hiểu URL cũ sẽ chuyển hướng qua alias, và sau lần đổi này slug sẽ bị khóa vĩnh viễn, không thể đổi thêm.</span></label>}<div className="md:col-span-2"><label className="text-sm text-gray-600">Ảnh đại diện</label><ImageUrlInput value={form.avatar_url} onChange={value => set('avatar_url', value)} folder="profiles" isAdmin={isAdmin} /></div><Field label="Số điện thoại công khai" value={form.public_phone} onChange={value => set('public_phone', value)} /><Field label="Zalo công khai" value={form.public_zalo} onChange={value => set('public_zalo', value)} />{isAdmin && <label className="text-sm text-gray-600">Trạng thái<select value={form.status} onChange={e => set('status', e.target.value)} className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 bg-white text-gray-900"><option value="draft">Bản nháp</option><option value="published">Đang xuất bản</option><option value="disabled">Đã tắt</option></select></label>}</div><label className="block text-sm text-gray-600 mt-3">Giới thiệu<textarea value={form.bio} onChange={e => set('bio', e.target.value)} rows={5} maxLength={2000} className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm resize-y" /></label><div className="flex items-center justify-between mt-4"><p className="text-xs text-gray-400">{isAdmin ? 'Admin có thể thay đổi trạng thái xuất bản.' : 'Staff chỉ chỉnh nội dung hồ sơ của chính mình.'}</p><button disabled={busy} onClick={() => void save()} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold disabled:opacity-50">{busy ? 'Đang lưu...' : 'Lưu hồ sơ'}</button></div></div>;
}
...
function Field({ label, value, disabled = false, onChange }: { label: string; value: string; disabled?: boolean; onChange: (value: string) => void }) { return <label className="text-sm text-gray-600">{label}<input disabled={disabled} value={value} onChange={e => onChange(e.target.value)} className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 disabled:bg-gray-100 disabled:text-gray-500" /></label>; }
