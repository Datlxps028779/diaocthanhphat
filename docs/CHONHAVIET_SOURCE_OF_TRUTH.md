...
**Đã làm/có bằng chứng:** metadata, canonical, Open Graph, Twitter, schema, sitemap và robots; active-only sitemap; quality-gated area/neighborhood pages; source/citation/FAQ foundation. News publication boundary production: migration `20260909030000_news_publish_boundary.sql` (`content_version`, `news_publication_events`, RPC `publish_news_article`); Vercel `NEWS_PUBLISH_BOUNDARY_MODE=enforce`; create/update luôn nháp; publish/unpublish owner-MFA; cổng SEO–GEO–AIO (tối thiểu 3 cụm từ khóa AIO, không kẹp trần 3–6; ≥2 internal link `href="/..."`; FAQ/citation floor). Admin hiện toàn bộ lỗi chặn một lần. Evidence 2026-09-11: bài `doanh-nghiep-bat-dong-san-xoay-dong-tien-khi-suc-mua-suy-yeu` đăng lại thành công trên production sau enforce. Batch A2: Admin filter “Chưa sẵn sàng đăng lại”, badge theo bài và editor liệt kê đủ blocker khi xem bài đã đăng — cùng rule A1, không chặn lưu bài live, không mutate content. Follow-up hardening migration service-role-only RPC `20260911010000_harden_news_publication_server_boundary.sql` đã được áp dụng production và preflight read-only xác nhận: RPC mới tồn tại, chỉ `service_role` được execute, browser roles không được execute RPC cũ/mới, 79/79 news có `content_version` hợp lệ. Production read-only 2026-09-11: 80 news (79 published, 1 draft); SQL proxy 38 thiếu `href="/..."` trong `news.content`, 48 citation ngoài 2–6, 40 FAQ ngoài 4–6, 9 keyword < 3; `area_id` 18/79. Chrome production: 77/79 published chưa sẵn sàng đăng lại. SEO Freshness Queue observability cũng đã hậu kiểm bằng Chrome và SQL read-only: 232 `succeeded`, không có `pending`/`processing`/`failed`/`dead_letter`; refresh chỉ đọc, không mutation. Horizon 4 read-only measurement ngày 2026-09-11: 79 News published, 78 slug canonical hợp lệ, 1 slug kết thúc bằng dấu gạch nối, 0 slug trùng, 0 location hierarchy mismatch; Search Visibility không thiếu row và không mismatch canonical URL/path cho 78 slug hợp lệ; duplicate canonical = 0. Local sitemap fix dùng chung canonical-slug validator với Search Visibility và không còn fallback sang raw slug/News ID cho source slug không hợp lệ; không sửa row production.

**Chưa làm đủ:** Search Console/indexation production evidence; structured entity content; sửa coverage internal-link/FAQ/citation trên corpus (đã đo, chưa rewrite); image/semantic HTML cleanup dựa trên measurement.
