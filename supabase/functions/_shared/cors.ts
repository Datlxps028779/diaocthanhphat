...
export async function verifyAdmin(req: Request, createClient: (u: string, k: string, options?: any) => any): Promise<string | null> {
...
export async function verifyAdminOrStaff(req: Request, createClient: (u: string, k: string, options?: any) => any): Promise<string | null> {
  const auth = req.headers.get("Authorization") ?? "";
  const token = auth.replace(/^Bearer\s+/i, "");
  if (!token) return null;
  const url = Deno.env.get("SUPABASE_URL")!;
  ...
  const { data: { user } } = await authClient.auth.getUser();
  if (!user) return null;
  const { data, error } = await authClient.rpc("is_admin_or_staff");
  return !error && data === true ? user.id : null;
}
