...
DROP POLICY IF EXISTS "public_media_insert_user_listing" ON storage.objects;
CREATE POLICY "public_media_insert_user_listing" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (
    bucket_id = 'public-media'
    AND name LIKE ('user-listings/' || auth.uid()::text || '/%')
  );
...
DROP POLICY IF EXISTS "public_media_update_user_listing" ON storage.objects;
CREATE POLICY "public_media_update_user_listing" ON storage.objects
  FOR UPDATE TO authenticated
  USING (
    bucket_id = 'public-media'
    AND owner = auth.uid()
...
DROP POLICY IF EXISTS "public_media_delete_user_listing" ON storage.objects;
CREATE POLICY "public_media_delete_user_listing" ON storage.objects
  FOR DELETE TO authenticated
  USING (
    bucket_id = 'public-media'
    AND owner = auth.uid()
